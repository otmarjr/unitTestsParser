...
namespace unitTestsParser
{
	public static class ReflectionHelper
	{
        ...
        public class MSILtoCSharpConverter
        {
            ...
            private static IEnumerable<CodeStatement> ToAST(Instruction method, IList<CodeExpression> parameters, IDictionary<int, CodeVariableDeclarationStatement> variables)
            {
                ...
                while (method != null)
                {
                    switch (method.OpCode.Code)
                    {
                        ...
                        case Code.Newobj:
                            {
                                var ctorMethod = (MethodReference)method.Operand;
                                var methodParams = ctorMethod.Parameters.Select(t => stack.Count > 0 ? stack.Pop() ?? new CodeExpression() : new CodeExpression()).ToArray();
